...
namespace ams::mitm::fspusb::impl {
    ...
    void SCSIBuffer::Write64BE(u64 val)
    {
        ...
        std::memcpy(&this->storage[this->idx], &tmpval, sizeof(u64));
        this->idx += sizeof(u64);
    }
    ...
    void SCSIDevice::AllocateBuffers() {
        if (this->buf_a == nullptr) {
            this->buf_a = (u8*)AllocateUSBTransferMemoryBlock(1);
        }
        if (this->buf_b == nullptr) {
            this->buf_b = (u8*)AllocateUSBTransferMemoryBlock(USB_TRANSFER_MEMORY_MAX_MULTIPLIER);
        }
        if (this->buf_c == nullptr) {
            this->buf_c = (u8*)AllocateUSBTransferMemoryBlock(1);
        }
    }
    ...
    SCSICommandStatus SCSIDevice::ReadStatus() {
        ...
        std::memset(&status, 0, sizeof(SCSICommandStatus));
                ...
                if (buffer != nullptr && transfer_length > 0) {
                    if (c.GetDirection() == SCSIDirection::In) {
                        while(total_transferred < transfer_length) {
                            ...
                            std::memcpy(buffer + total_transferred, this->buf_b, transferred);
                        ...
                        while(total_transferred < transfer_length) {
                            ...
                            std::memcpy(this->buf_b, buffer + total_transferred, cur_transfer_size);
    ...
    void SCSIContext::Initialize() {
                ...
                switch(sense_key)
                {
                    ...
                    case SCSI_SENSE_NOT_READY:
                        if (sense_key == SCSI_SENSE_NOT_READY) {
                            ...
                            svcSleepThread(3'000'000'000ul);
                        }
        ...
        if (this->ok && status.status == SCSI_CMD_STATUS_SUCCESS) {
            ...
            if (this->ok && status.status == SCSI_CMD_STATUS_SUCCESS) {
                ...
                std::memcpy(&lba_bytes, &read_capacity_10_response[0], 4);
                ...
                if (size_lba > 0 && size_lba < SCSI_MAX_BLOCK_10) {
                    ...
                    std::memcpy(&lba_bytes, &read_capacity_10_response[4], 4);
                    ...
                    if (this->ok && status.status == SCSI_CMD_STATUS_SUCCESS) {
                        ...
                        std::memcpy(&size_lba, &read_capacity_16_response[0], 8);
                        size_lba = __builtin_bswap64(size_lba);
                        
                        std::memcpy(&lba_bytes, &read_capacity_16_response[8], 4);
                        lba_bytes = __builtin_bswap32(lba_bytes);
